...
for(var i=0;i<playerArrows.length;i++){
  var baseCol=Matter.SAT.collides(
    playerArrows[i].body,
    computerBase.body
  );
  var archerCol=Matter.SAT.collides(
    playerArrows[i].body,
    computerArcher.body
  );
  var computerCol=Matter.SAT.collides(
    playerArrows[i].body,
    computer.body
  );

  if(baseCol.collided || archerCol.collided || computerCol.collided){
...
function handleComputerArrowCollision() {
  ...
  for(var i=0;i<computerArrows.length;i++){
    var CbaseCol=Matter.SAT.collides(
      computerArrows[i].body,
      playerBase.body
    );
    var CarcherCol=Matter.SAT.collides(
      computerArrows[i].body,
      playerArcher.body
    );
    var playerCol=Matter.SAT.collides(
      computerArrows[i].body,
      player.body
    );
  
    if(CbaseCol.collided || CarcherCol.collided || playerCol.collided){
